...
public class MapCommands {
    ...
    public static Format<MapCommand> getDefaultFormat(MapController mapController) {
        return new ExtensibleMapCommandFormat()
                .put("(\\d+)\\s*x\\s*(\\d+)((\\s*\\p{Alpha}((?:\\d+)?(?:,\\d+)?)?)?(?:\\+\\+|--)?)?", m -> {
                    ...
                    final Room parent = mapController.getCurrentRoom();
                ...
                .put("s(?:tairs)?\\s*(\\d+)(\\p{Alpha})", m -> new AddStairsCommand(getWallPointer(mapController, m.group(1), m.group(2))))
                .put("d(?:oor)?\\s*(\\d+)(\\p{Alpha})", m -> new AddDoorCommand(getWallPointer(mapController, m.group(1), m.group(2))))
                .put("(\\d+)?(\\p{Alpha})", m -> new GoTowardCommand(getWallPointer(mapController, m.group(1), m.group(2))))
    ...
    private static WallLocation getWallPointer(MapController mapController, String indexStr, String directionStr) {
        ...
        final int index = indexStr != null
                ...
                : getMiddleIndex(direction, mapController.getCurrentRoom());
    ...
    private static RelativeRoomLocation getRoomLocation(Room parent, String locationString, Integer x, Integer y) {
        ...
        final int childIndex = Optional.ofNullable(locMatcher.group(3))
                .map(str -> Integer.parseInt(str) - 1)
                .orElseGet(() -> locMatcher.group(2) == null ? getMiddleIndex(direction, x, y) : 0);
    ...
    private static int getMiddleIndex(CardinalPoint direction, double x, double y) {
